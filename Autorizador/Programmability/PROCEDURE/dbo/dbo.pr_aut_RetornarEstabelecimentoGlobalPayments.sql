...
CREATE PROCEDURE [dbo].[pr_aut_RetornarEstabelecimentoGlobalPayments] (
	@sEstabelecimento VARCHAR(20) OUTPUT
	,@cStatus CHAR(1) OUTPUT
	,@bEstabMigrado BIT OUTPUT
	,@bHabilitadoGlobalPayments BIT OUTPUT
	)
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN
		SELECT @iRetorno = E.CodigoEstabelecimento
			,@bEstabMigrado = 1
			,@bHabilitadoGlobalPayments = ISNULL(ES.Habilitado, 0)
			,@cStatus = CASE 
		...
		INNER JOIN Acquirer.dbo.EstabelecimentoGlobalPayment ES WITH (NOLOCK) ON (ES.CodigoEstabelecimento = E.CodigoEstabelecimento)
		WHERE ES.CodigoGlobalPayment = @iEstabelecimento
		...
		BEGIN
			...
			INNER JOIN Acquirer.dbo.EstabelecimentoGlobalPayment ES WITH (NOLOCK) ON (ES.CodigoEstabelecimento = E.Numero)
			WHERE ES.CodigoGlobalPayment = @iEstabelecimento
		END
	END
