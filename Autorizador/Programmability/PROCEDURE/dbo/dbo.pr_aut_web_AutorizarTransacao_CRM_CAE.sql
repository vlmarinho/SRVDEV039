...
Chamado: 1812108
Descrição: Altera tipo do parâmetro @cNumero_Parcelas de CHAR(2) para VARCHAR(2). O tipo 
	anterior causava falha na tratativa do número de transações dentro do autorizador.
--------------------------------------------------------------------------
Data: 12/03/2024
Autor: Adilson Pereira - Up Brasil
Chamado: 2111779
Descrição: Incluir a resposta de recarga no bit 63 para evitar que erros no parse
	da resposta impeçam o desfazimento de transação negada pelo operadora.
-------------------------------------------------------------------------- 	
*/

CREATE PROCEDURE [dbo].[pr_aut_web_AutorizarTransacao_CRM_CAE] (
...
BEGIN
	...
	DECLARE
		...
		,@iBitRespRecarga	INT = 0
		,@cBitRespRecarga	VARCHAR(1000)
		,@bTransDesfeita	BIT = 0
	...
	BEGIN TRY	
		...
		IF (CONVERT(INT,@cRedeCaptura) IN ( 16, 17, 27, 37))
		...
		ELSE IF CONVERT(INT,@cRedeCaptura) = 37
			SET @cResponsavel = 'SGF/' + @cResponsavel
		...
		SET @cBitRespRecarga = [DBO].[f_RemoverSinalizador](@Bit063)
		IF LEN(@cBitRespRecarga) > 0 AND ISNUMERIC(@cBitRespRecarga) = 1
		BEGIN
			SET @iBitRespRecarga = CONVERT(VARCHAR, @cBitRespRecarga)
		END
		...
		BEGIN
			...
			IF (@iResposta = 0 AND @iBitRespRecarga > 0)
			BEGIN
	
				SET @bDesfazTransacao = 1
				SET @iResposta = @iBitRespRecarga
	...
	BEGIN CATCH
		...
		BEGIN
			...
			IF (@iResposta = 0 AND @iBitRespRecarga > 0 AND @bTransDesfeita = 0)
			BEGIN
