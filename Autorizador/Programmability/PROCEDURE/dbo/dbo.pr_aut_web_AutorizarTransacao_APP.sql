/*
 ...
 Autor: Adilson Pereira - Up Brasil
 Chamado: 2005115 
 Descrição: Alterar consulta de transação a ser estornada, para buscar pelo NsuHost,
 	no caso de transações convênio
 --------------------------------------------------------------------
 */

CREATE PROCEDURE [dbo].[pr_aut_web_AutorizarTransacao_APP] ( 
...
BEGIN 
	...
	BEGIN 
		...
		ELSE  
		BEGIN 
			SET @cTrnEstornar = (SELECT CONVERT(VARCHAR,NsuLoja) + DataHoraGMT FROM   Policard_603078.dbo.Transacao_RegistroTEF WITH(NOLOCK) WHERE NsuHost = @cNSUPolicard) 
	...
	print concat('@Bit039: ', @Bit039)
	print concat('@Bit062: ', @Bit062)
	...
	BEGIN 
		...
		BEGIN 
			...
			BEGIN 
				...
				SET @sComprovante		= SUBSTRING(@Bit062,1,(CHARINDEX('|',@Bit062)-1)) SET @Bit062 = SUBSTRING(@Bit062, (CHARINDEX('|',@Bit062)+1), LEN(@Bit062))		 
				IF @cBaseOrigem = 'C'
				BEGIN
					SET @cSaldo 	= SUBSTRING(@Bit062,1,(CHARINDEX('|',@Bit062)-1)) SET @Bit062 = SUBSTRING(@Bit062, (CHARINDEX('|',@Bit062)+1), LEN(@Bit062)) 
				END
