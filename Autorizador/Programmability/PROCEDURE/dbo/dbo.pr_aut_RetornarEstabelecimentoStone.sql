...
Objeto......: [pr_aut_RetornarEstabelecimentoStone]
...
CREATE PROCEDURE [dbo].[pr_aut_RetornarEstabelecimentoStone] (
	 @sEstabelecimento	VARCHAR(20)	OUTPUT
	,@cStatus			CHAR(1)		OUTPUT
	,@bEstabMigrado		BIT			OUTPUT
	,@bHabilitadoStone	BIT			OUTPUT
	)
AS
BEGIN
	...
	BEGIN
		...
		SELECT @iRetorno = E.CodigoEstabelecimento
			  ,@bEstabMigrado = 1
			  ,@bHabilitadoStone = ISNULL(ES.Habilitado,0)
		...
		INNER JOIN Acquirer.dbo.EstabelecimentoStone ES WITH(NOLOCK) ON (ES.CodigoEstabelecimento = E.CodigoEstabelecimento)
		WHERE ES.CodigoStone = @iEstabelecimento
		AND es.Habilitado = 1
		...
		BEGIN
			...
			INNER JOIN Acquirer.dbo.EstabelecimentoStone ES WITH(NOLOCK) ON (ES.CodigoEstabelecimento = E.Numero)
			WHERE ES.CodigoStone = @iEstabelecimento
			AND es.Habilitado = 1
